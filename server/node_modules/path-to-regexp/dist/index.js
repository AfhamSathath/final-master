...
function parse(str, options = {}) {
    ...
    while (index < chars.length) {
        ...
        if (type) {
            ...
            // FIX: unnamed wildcards allowed
            let wildcardName = "";
            if (index + 1 < chars.length && ID_START.test(chars[index + 1])) {
                index++; // move past *
                wildcardName = name();
            } else {
                index++; // move past *
            }
            tokens.push({ type: "wildcard", index, value: wildcardName });
        } else {
    ...
    function consumeUntil(endType) {
        ...
        while (true) {
            ...
            if (token.type === "char" || token.type === "escape") {
                ...
                let cur = tokens[pos];
                while (cur && (cur.type === "char" || cur.type === "escape")) {
                    path += cur.value;
                    cur = tokens[++pos];
                }
            ...
            if (token.type === "param" || token.type === "wildcard") {
                output.push({ type: token.type, name: token.value });
                continue;
            }
            if (token.type === "{") {
                output.push({ type: "group", tokens: consumeUntil("}") });
                continue;
            }
